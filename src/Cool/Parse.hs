...
-- TODO: inline
-- {-# INLINE identChar #-}
module Cool.Parse (coolLexer, parseStringWith, parseFileWith, printLexResult) where
...
coolLexer = spaceConsumer >> (many $ do
  ...
  choice $ [
    ...
    ++ (map ((tag p ReservedWord <$>) . reservedWord)       reservedWords)
    ++ (map ((tag p ReservedWord <$>) . reservedIdentifier) reservedIdentifiers)
    ++ (map ((tag p Symbol       <$>) . symbol)             reservedSymbols))
    <* eof
